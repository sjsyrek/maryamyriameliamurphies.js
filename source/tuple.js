 ...
 * fst(tup);                  // => 10
 */
export function fst(p) { return isTuple(p) ? p[1] : error.tupleError(p, fst); }
 ...
 * snd(tup);                  // => 20
 */
export function snd(p) { return isTuple(p) ? p[2] : error.tupleError(p, snd); }
 ...
 * swap(tup);                 // => (20,10)
 */
export function swap(p) { return isTuple(p) ? tuple(snd(p), fst(p)) : error.tupleError(p, swap); }
 ...
 */
export function isTuple(a) { return a instanceof Tuple && a !== unit ? true : false; }
 ...
 */
export function fromArrayToTuple(a) {
 ...
 */
export function fromTupleToArray(p) {
